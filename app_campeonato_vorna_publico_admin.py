...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        st.subheader("Acesso")
        admin_mode = st.toggle("Modo admin", value=False)
        admin_ok = False
        if admin_mode:
            admin_password = st.text_input("Senha admin", type="password")
            # Em produção, defina em Secrets: ADMIN_PASSWORD = "sua_senha"
            expected_password = st.secrets.get("ADMIN_PASSWORD", "vorna_admin")
            admin_ok = admin_password == expected_password
            if admin_password and not admin_ok:
                st.warning("Senha admin inválida.")
            elif admin_ok:
    ...
    if admin_mode and admin_ok:
        with st.expander("🔐 Auditoria admin dos dados"):
            valid_dep = inout[
                (inout["success"])
                & (inout["is_deposit"])
                & (inout["amount"] > 0)
            ...
            audit_aff = st.text_input("Auditar Aff ID específico", key="audit_aff_id")
            if audit_aff.strip():
                dep_a = valid_dep[valid_dep["aff_id"].astype(str).str.strip() == audit_aff.strip()].copy()
                tr_a = valid_tr[valid_tr["aff_id"].astype(str).str.strip() == audit_aff.strip()].copy()
                ftd_a = ftds[ftds["aff_id"].astype(str).str.strip() == audit_aff.strip()].copy()

                st.subheader(f"Resumo auditado — Aff ID {audit_aff.strip()}")
                ac1, ac2, ac3 = st.columns(3)
                ac1.metric("FTDs válidos", int(ftd_a["user_id"].nunique()) if not ftd_a.empty else 0)
                ac2.metric("Depósitos válidos", usd(dep_a["amount"].sum()))
                ac3.metric("Turnover", usd(tr_a["turnover"].sum()))

                dep_points = int(dep_a["amount"].sum() // RULES["points_deposit_step"]) * RULES["points_per_deposit_step"]
                turnover_points = int(tr_a["turnover"].sum() // RULES["points_turnover_step"]) * RULES["points_per_turnover_step"]
                ftd_points = (int(ftd_a["user_id"].nunique()) if not ftd_a.empty else 0) * RULES["points_per_ftd"]
                st.info(f"Cálculo bruto do período: FTD {ftd_points} pts + Depósitos {dep_points} pts + Turnover {turnover_points} pts = {ftd_points + dep_points + turnover_points} pts")

                st.write("Depósitos válidos")
                st.dataframe(dep_a[["date", "user_id", "aff_id", "transaction_type", "amount", "success"]], use_container_width=True, hide_index=True)

                st.write("Trades considerados")
                st.dataframe(tr_a[["date", "user_id", "aff_id", "turnover"]], use_container_width=True, hide_index=True)

                st.write("FTDs válidos")
                st.dataframe(ftd_a[["ftd_date", "user_id", "aff_id", "month"]], use_container_width=True, hide_index=True)
            else:
    ...
    elif admin_mode and not admin_ok:
        st.warning("Auditoria disponível apenas com senha admin.")
    ...
    st.caption("Observação: ranking aberto para acompanhamento dos afiliados. Auditoria detalhada disponível apenas no modo admin.")
