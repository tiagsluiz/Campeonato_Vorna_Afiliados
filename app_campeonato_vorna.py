...
def load_workbook(source_url: str, uploaded_file) -> Dict[str, pd.DataFrame]:
    if uploaded_file is not None:
        return pd.read_excel(uploaded_file, sheet_name=None, engine="openpyxl")
    return pd.read_excel(google_export_url(source_url), sheet_name=None, engine="openpyxl")
...
def main():
    ...
    st.caption("Período oficial: Maio a Outubro de 2026 | Regras configuradas conforme regulamento")

    with st.sidebar:
        st.header("Fonte de dados")
        source_url = st.text_input("Link do Google Sheets", SHEET_URL_DEFAULT)
        uploaded = st.file_uploader("Ou envie o Excel manualmente", type=["xlsx"])
        st.header("Filtros")
    ...
    try:
        with st.spinner("Carregando e processando dados..."):
            sheets = load_workbook(source_url, uploaded)
            inout, trades, users = prepare_data(sheets)
            monthly, final, ftds = build_metrics(inout, trades)
    except Exception as e:
        st.error("Não consegui carregar/processar a planilha. Veja o erro abaixo e confira se o Google Sheets está compartilhado ou use upload manual do Excel.")
        st.exception(e)
        st.stop()
    ...
    with st.expander("Resumo das regras aplicadas no cálculo"):
        st.markdown("""
        - Temporada considerada: **01/05/2026 a 31/10/2026**.
